...
class SpannerQueue extends Queue implements QueueContract
{
    /**
    ...
    public function bulk($jobs, $data = '', $queue = null)
    {
        ...
        return $this->database->table($this->table)->insert(collect((array)$jobs)->map(
            function ($job) use ($queue, $data, $availableAt) {
                return $this->buildDatabaseRecord(Str::orderedUuid()->toString(), $queue, $this->createPayload($job, $this->getQueue($queue), $data), $availableAt);
            }
        )->all());
    }
    ...
    protected function pushToDatabase($queue, $payload, $delay = 0, $attempts = 0)
    {
        ...
        return $this->database->table($this->table)->insert($this->buildDatabaseRecord(
            $id, $this->getQueue($queue), $payload, $this->availableAt($delay), $attempts
        ));
    }
